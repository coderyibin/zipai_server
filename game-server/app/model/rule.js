    ...
    for (i = 0; i < userList.length; i++) {
        ...
        if(!! matchList) {
            ...
            if(!! canHu) {
                ...
                sendClient = this.filterNoCard(sendClient,player.handCardList,player.uid);
                return sendClient;
            }
            //扫穿
            sendClient[player.uid] = [];
...
//清除当下去之后手上没牌的情况 handlList玩家手牌  uid玩家uid
module.exports.filterNoCard = function(obj,handleList,uid){
    if(handleList.length > 7) {
        return obj;
    }
    var childArr = obj[uid];
    var childObj;
    var calcIndex;
        ...
        for(var i = 0;i<childArr.length;i ++){
            ...
            if(childObj.cardArr.length == 1){
                if(childObj.cardArr[0].length == handleList.length){
                    ...
                    var cloneList = handleList.slice(0);
                    var nowCardArr = childObj.cardArr[0];
                    var cardIndex;
                    for(var j = 0;j < nowCardArr.length;j ++){
                        cardIndex = cloneList.indexOf(nowCardArr[j]);
                        if(cardIndex != -1){
                            cloneList.splice(cardIndex,1);
                        }
                    }
                    var same = true;
                    var tempCardId = cloneList[0];
                   for(var k = 0;k < cloneList.length;k ++){
                        if(cloneList[k] != tempCardId){
                            same =false;
                        }
                    }
                    console.log("same" + same +" cloneLength");
                    if(same && cloneList.length > 2){
                        delete  obj[uid];
                        calcIndex = 0;
                        for(var prop in obj){
                            calcIndex ++ ;
...
module.exports.checkPeng = function(cardId,cardIdList) {
    var resultRule = this.checkRule(cardIdList,[[cardId,cardId]]);
    return resultRule;
}
...
module.exports.calcHu = function(minHu,player,cardId,currPlayerUid,Auto,spliceEnd,fourAgain) {
    ...
    if(!! huBool) {
        //能胡判断胡数
        isMine = (currPlayerUid == player.uid);
        totalHuShu = this.calcListScore(huBool,isMine,Auto,cardId);
        //判断胡数是否足够
