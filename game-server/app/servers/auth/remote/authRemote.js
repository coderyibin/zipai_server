...
var Remote = function(app) {
    ...
    this.formalAccout = app.get("formalAccount").accoutList;
};
...
pro.auth = function(authCode,token,openId,refreshToken,username,ipAddress,isPublicWeb,cb) {
    ...
    if(!! token){
        ...
        authLogger.info(" ahve token to author:" + authUrl);
        ...
        authLogger.info(" only Code to Requset:" + authUrl);
        request(authUrl, function (error, response, body) {
            if(error) {
...
{
    if(this.isDebug) {
        if(this.app.get('curServer').formal == 1){
            if( this.formalAccout.indexOf(openId) == -1){
                cb(null, Code.ENTRY.DEBUG_LOGIN_FAIL);
                return;
             }
        }
