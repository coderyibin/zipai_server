...
Handler.prototype.getSignature = function(msg,session,next){
    ...
    fightLogger.info("get wxpWebhandler/getSignatrue:" + url);
    roleMgr.getUserInfo(session,session.uid,function(roleData){
        roleData.isPublicWeb = true;
        if(! roleData.isPublicWeb){
            next(null,Code.FAIL);
        } else{
            gameMgr.getPWebTickt(url,function(code,params){
                console.log("wxPWebHandler getSignature" + util.inspect(params));
                next(null,{
                    code:code,
                    data:params
                });
                fightLogger.info("get wxpWebhandler/getSignatrue result:" + util.inspect(params));
            });
