...
userDao.registerUser = function(user,params,cb) {
	...
	if(! user){
		...
		if(userDao.isDebugPlayer(user.openid)){
			...
			sql = 'update user set nickname = ?,sex = ?,province = ?,city = ?,country = ?,headimgurl = ?,lastLoginDate = ? where openid = ?';
			args = [params.nickname,params.sex,params.province,params.city,params.country,params.headimgurl,nowDate,params.openid];
		}
	}
	logger.info("player reigster User sql:" + sql);
