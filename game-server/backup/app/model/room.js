...
var GameRoom = function (roomId,channelService) {
    ...
    this.cardEffectTimeID = null; //房间牌面效果等待时间
...
Prototype.startCardEffect = function(callback){
    this.clearCardEffect();
    this.cardEffectTimeID = setTimeout(callback,5000,this);
}
...
//获取房间玩家简要信息回调给客户端
Prototype.getClientUserInfo = function()
{
    var obj = {};
    for(var i = 0;i < this.userList.length;i ++) {
...
Prototype.filterWaitUIList = function() {
    parent: for (var i = 0; i < this.waitUidList.length; i++) {
        child: for(var j = 0;j < this.userList.length;j ++) {
            if(this.userList[j].uid == this.waitUidList[i] && this.userList[j].isLost) {
