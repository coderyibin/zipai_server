...
module.exports = function(app, http) {
	...
	http.post('/unifiedOrder', function(req, res) {
		...
		var wxpay = pomelo.app.get('WXPay');

		var appid =  wxpay.get("appid");
		var mch_id =  wxpay.get("mch_id");
		var partner_key =  wxpay.get("partner_key");
		var nonce_str = util.generateNonceString();

		var total_fee = req.body.total_fee;
		var out_trade_no = util.generateOutTradeNo();

		logger.info("unifiedOrder out_trade_no*******************===" + out_trade_no);
			...
			if (err) {
				...
				if (result.return_code == "SUCCESS" && result.result_code == "SUCCESS") {
					...
					res.send({
						...
						result : {'appid':appid, 'mch_id': mch_id, 'prepay_id': result['prepay_id'], 'nonce_str': nonce_str, 'time_stamp': timeStamp, 'package_value':'Sign=WXPay', 'sign': sign , "out_trade_no" : out_trade_no}
					})
				}else{
					res.send({
						code : 500,
